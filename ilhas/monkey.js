function monkeyUpdate(){
	...
	if(monkey.active){
		if(monkey.path.length <= 0){
		...
		} else if(Math.floor(monkey.x) == monkey.path[0][0]*32 && Math.floor(monkey.y) == monkey.path[0][1]*32){
			monkey.path.shift()
		} else {
			if(Math.floor(monkey.x) < monkey.path[0][0]*32){
				monkey.x+=speed*dt
			}
			...
			if(Math.floor(monkey.y) < monkey.path[0][1]*32){
				monkey.y+=speed*dt
			}
			if(overlaps(player[0].x, player[0].y, 32, 32, monkey.x, monkey.y, 32, 32)){
				monkey.y-=speed*dt
