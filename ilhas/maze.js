let bananas = [{x:20*32, y: 18*32}, {x: 10*32, y: 22*32}, {x: 44*32, y: 23*32}] 
function mazeUpdate(){
	...
	sprite('monkey.png', guepard.x, guepard.y-32, 1, 1, 0, 0, width('monkey.png'), height('monkey.png'), player[4].camX, player[4].camY, mapName)
	bananas.forEach((e, i) => {
		if(overlaps(player[4].x, player[4].y, 32, 32, e.x, e.y, 32, 32)){
			bananas.splice(i, 1)
		}
		sprite('banana.png', Math.min(e.x, e.x - player[4].camX), Math.min(e.y, e.y - player[4].camY))
	})
	if(bananas.length == 0){
		room  = gameMapUpdate
		if(!played){
