...
function gameMapUpdate(){
	...
	render('gameMap0' +'Layer1.tls',player[0].camX, player[0].camY)

	sprite('gameTrees.png',Math.min(0, -player[0].camX-48), Math.min(0, -player[0].camY+72))
	houses.forEach((e, i) => {
		houseUpdate(e, 0)
	})
	if(mapName == 'gameMap0'+'.tls'){
		statues.forEach((e, i) => {
			statueUpdate(e, 0)
		})
		if(stars >= 3)questionsTeacherUpdate(0)
		playerUpdate(0)
		//teacherUpdate(0)
		//write(tickets, 640-textWidth(tickets, '2rem slkscr')-32, 8, 'white', '2rem slkscr')
	}
	render('gameMap0' +'.tls', Math.min(old.camX, width('gameMap0.tls')*32-width()), old.camY)
	...
	if(timerValue(0) > 100 && (starFrameX/136-1)%13!=0){
		starFrameX+=136
		timerStart(0)
	}
}