let justClicked, write, tx, ty, sprite, render, key, keys, clear, axisX, axisY, timers = [], timerStart, timerValue, random, width, height, dirX, dirY, centerX, centerY;
let assets = {}
let UIs = []
...
async function assetsLoad(){
	...
	for(let i = 0; i < arr.length; i++){
		...
		if(el.slice(-4) == '.tls'){
			...
			let map = Array.from({length:height}).map((e, i) => lines[i+1].split(','))
			assets[el] = {walkable: {}, path: {}, tiles: map, width: map[0].length, height: map.length}
	...
	render = (path, x, y, x2, y2, sx, sy) => {
		...
		assets[path].tiles.slice(y/32, (y+height()+32)/32).forEach((e, i) => {
			 e.slice(x/32, (x+width()+32)/32).forEach((e2, i2) => {
			 	sprite(assets[path].path[e2], (x2+i2*32-x%32)*sx, (y2+i*32-y%32)*sy, sx, sy)
			 })	
		})
	}
	...
	overlaps = (ax, ay, aw, ah, bx, by, bw, bh) => {
		if(bx.length != undefined){
			let res = false
			if(bx.slice(-4) == '.tls'){
		        let y = Math.min(Math.max(ay, 0), Math.max(height(bx)*32-height(), 0))
        		let x = Math.min(Math.max(ax, 0), Math.max(width(bx)*32-width(), 0))
