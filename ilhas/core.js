...
async function assetsLoad(){
	...
	overlaps = (ax, ay, aw, ah, bx, by, bw, bh) => {
		if(bx.length != undefined){
			let res = false
			if(bx.slice(-4) == '.tls'){
		        let y = Math.min(Math.max(ay-height()/2-ah/2, 0), Math.max(height(bx)*32-height(), 0))
        		let x = Math.min(Math.max(ax-width()/2-aw/2, 0), Math.max(width(bx)*32-width(), 0))
        		assets[bx].tiles.slice(y/32, (y+height()+32)/32).forEach((e, i) => {
            		e.slice(x/32, (x+width()+32)/32).forEach((e2, i2) => {
						if(assets[bx].walkable[e2] == 'false'){
							res = res || overlaps(ax, ay, aw, ah, i2*32+x, i*32+y, 16, 16)
						}
            		})
            	})
			} else {
